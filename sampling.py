...
class Sampling_Model(object):
    """This class models the sampling model,
    given the data, types of inputs, k, n, and s
    """

    def __init__(self, data, input_types, header,k, n, s):
        """Constructor.
        ...
        :param k:
            the k in the sampling algorithm
        :param n:
            the n in the sampling algorirthm, how many elements to set as outlier
        :param s:
            the s in the sampling algorirthm, how many elements to take as a sample
        ...
        self.filtered_data = []
        self.filtered = False
        self.sample_set = numpy.zeros(shape=(s,len(self.data[0])))
        self.running_time = 0
    def run(self): # function called by get_n_outliers function;
        ...
        #myRange = range(0,self.s)
        ci=0
        myRange = random.sample(range(0, len(self.data)), self.s)
        for count in myRange:
            self.sample_set[ci] =  self.data[ count ]
        start = time.time()
        ...
        print "Finding outliers using one time sampling took {} seconds!".format(time.time() - start)   
